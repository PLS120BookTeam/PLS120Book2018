...
When designing an experiment with blocks, the goal is to provide uniformity of the experimental unit within blocks. For example, if performing an experiment on clams, grouping clams according to size class would be an appropriate characteristic to group. Alternatively, for plants, a grouping characteristic may be equal-sized plots within the field that span a known variation in the topography, such as gradual slope. Once experimental units have been seperated into blocks, treatments are assigned randomly within a block, for each block. For example, say juvenile clams at three different size classes (1--2, 4--5, 8--9 mm in shell length) were blocked and three treatment levels of temperature were appllied to examine feeding rates. In this design, 1 replicate at each treatment level (a-c) was randomly assinged to a block (\@ref(fig:blockdesign)).  

<br>

```{r blockdesign, message=FALSE, warning=FALSE, paged.print=FALSE, out.width = '50%', fig.align='center', echo=FALSE, fig.cap ="Juvenile clam feeding rates examined at (a) ambient temperature, (b) +2 °C, and (c) +5 °C."}

knitr::include_graphics("images/blockdesign.pdf")

```

<br>

## Model and Partitioning of Variance with Blocks

In analysis of variance we use data to calculate the total variation among observations: variation between groups and variation within groups. When we incorporate blocks into an ANOVA, variability *within* is now split between "blocks" and "within" (\@ref(fig:SSblock)). 

<br>

```{r SSblock, message=FALSE, warning=FALSE, paged.print=FALSE, out.width = '50%', fig.align='center', echo=FALSE, fig.cap ="Sum of squares breakdwon for ANOVA with randomized complete block design."}

knitr::include_graphics("images/SSblock.pdf")
...
<br>
\begin{align}
&Y_{ij} = \mu_{..} + \tau_i + \beta_j + \epsilon_{ij} \quad \quad i = 1, \dots, k, \quad j = 1, \dots, r \\[10pt]
...
&\beta_j \quad \text{is the effect of blocks j } \\[15pt]
...
<br>
\begin{align}
&Y_{ij} = \hat{\mu}_{..} + \hat{\tau}_i + \hat{\beta}_j + \hat{\epsilon}_{ij} \\[10pt]
&= \bar{Y}_{..} + \bar{Y}_{i.} + \bar{Y}_{j.} + e_{ij} \\[15pt]
{where} \\[15pt]
...
&\bar{Y}_{j.} \quad \text{is the average for blocks j} \\[15pt]
...
<br>
\begin{align}
Y_{ij} &= \bar{Y}_{..} + \hat{\tau}_i + \hat{\beta}_j + \hat{\epsilon}_{ij} \\[15pt]
Y_{ij}- \bar{Y}_{..} &= \hat{\tau}_i + \hat{\beta}_j + \hat{\epsilon}_{ij} \\[15pt]
Y_{ij}- \bar{Y}_{..} &= (\bar{Y}_{i.} - \bar{Y}_{..})+ (\bar{Y}_{j.} - \bar{Y}_{..}) + (\bar{Y}_{ij} - \bar{Y}_{i.} - \bar{Y}_{j.} ) \\[15pt]
(\#eq:obsPartCRD)
\end{align}
<br>
...
<br>
\begin{align}
\sum_{i=1}^k \sum_{j=1}^r (Y_{ij}- \bar{Y}_{..})^2 &= \sum_{i=1}^k \sum_{j=1}^r (\bar{Y}_{i.} - \bar{Y}_{..})^2 + \sum_{i=1}^k \sum_{j=1}^r (\bar{Y}_{j.} - \bar{Y}_{..})^2 + \sum_{i=1}^k \sum_{j=1}^r (\bar{Y}_{ij} - \bar{Y}_{i.} - \bar{Y}_{j.})^2 \\[15pt]
TSS &= SST + SSB + SSE\\[15pt]
\text{where} \\[15pt]
...
SSB &= \sum_{i=1}^k \sum_{j=1}^r (\bar{Y}_{j.} - \bar{Y}_{..})^2 = \ k \ \sum_{i=1}^k (\bar{Y}_{j.} - \bar{Y}_{..})^2\\[15pt]
SSE &= \sum_{i=1}^k \sum_{j=1}^r (\bar{Y}_{ij} - \bar{Y}_{i.} - \bar{Y}_{j.})^2\\[15pt]
(\#eq:ssPartCRD)
\end{align}
<br>
