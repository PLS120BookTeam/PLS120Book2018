...
RStudio have 4 panes in the window.  These windows and many other features of RStudio are explained <a href="https://www.rstudio.com/resources/webinars/rstudio-essentials-webinar-series-part-1/" target="new">here</a>. Make sure to watch the video as part of your homework and send questions to your instructors.

When RStudio is opened, there are four windows that appear : 
...
```{r ps boxplot1, echo = TRUE, include = TRUE}
...
```{r ps grouped, echo=TRUE, message=FALSE, warning=FALSE, include=TRUE}
...
heifer <- read_excel("Datasets/HeiferDataLab1.xlsx", col_names = TRUE) # read in data.

(avg.bwt <- mean(heifer$Birth_Wt)) # obtain the average bwtance; note the use of $ to select parts of an object and the outer parentheses to display the result.
