...
In general, for a table of values with i rows and j columns, we write:
...
\mathbf{Y}_{i,j} = 
 \begin{pmatrix}
  ...
  Y_{2,1} & Y_{2,2} & \cdots & Y_{2,j} \\
  \vdots  & \vdots  & \ddots & \vdots  \\
  Y_{i,1} & Y_{i,2} & \cdots & Y_{k,j} 
 \end{pmatrix}
(\#eq:math09)
 \end{equation} 
<br>
...
nrow(Ydata) # Ydata has 3 rows: i = 3

ncol(Ydata) # and 7 columns: j = 7
...
length(Ydata[2, ]) # This calculates the number of values in the corn row
