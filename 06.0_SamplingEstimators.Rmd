...
The symbol $\int_\mathbb{R}$ represents the *integral* of the function over all real values of Y, and $\int_{\mathbb{R}} f(y) \ dy$ is the integral between $Y = a$ and $Y = b$. If you are not familiar with the concept of integral, no worries. Imagine that $f(y)$ is an absolute continuous function of the value $y$. The integral between any two $y$ values a and b is the area under the curve between the two points (Figure \@ref(fig:Sum2Integral)). One way to calculate the area approximately is to divide the interval [a, b] into $(b-a)/dy$ little segments. The area of each segment is the base $dy$ times the height, which can be set at $f(y_{lo} + dy/2)$ where $y_{lo}$ is the left end of each segment.¬
...
(df <- data.frame(y = 0:7, P = ppois(q = 0:7, lambda = 10 = 1/6)))
plot(df, type = "h", col = "skyblue", lwd = 4)
...
What is the probability that $Y \sim N(5, 4)$ takes values between 6 and 9, or that it takes values lower than3? Those probabilities are represented as areas under the curve of $Y \sim N(5,4)$ and under the SND $Z \sim N(0,1)$ in Figure \@ref(fig:normDistFig2).
...
Frequently, we use the terms population and universe as if they were the same, and this does not cause much confusion. But keep in mind that a universe of objects can have as many populations as variables we can define in that universe. The distincion is relevant because we will have to use different distribution functions for the different populations. The number of electronic devices might be modeled with a Poisson distribution, but not with a normal distribution, because it is discrete and has a mean that is close to zero.
...
Suppose that we are interested in the square of people's height. This quantity is used in the calculation of the Body Mass Index. Further suppose that height follows a normal distribution with mean $\mu$ and variance $\sigma^2$. We can define the *population* in this example as all University of California Davis students. We can, therefore, assume that PLS120 is a random sample of UCD students as the course draws students from multpile disciplines. In a class of 150 students (n=150), the mean and variance can be calculated providing an estimate for the popualtion parameter \mu and \sigma^2. 

```{r}

n = (150 min =149, max = 160)
sample = rnorm(length(n), n, 2) # Generate 150 random observations within approximately realistic bounds

hist(n) #display histogram of random generated samples
...
1. Make a frequency table where two random variables are not correlated and are not independent. Use calculations to show both properties are as requested.
