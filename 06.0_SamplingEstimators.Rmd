...
- When you are asked for a probability, for continuous RV's use the R function starting with "p", as in `pnorm`.
...
As we will see in a later chapter, if treatments means are not different, the quotient of mean squares in analysis of variance is supposed to have an $\mathcal{F}$ distribution.

The PDF and CDF for the $\mathcal{F}$ distribution are complicated and not presented here.
...
Frequently, we use the terms population and universe as if they were the same, and this does not cause much confusion. But keep in mind that a universe of objects can have as many populations as variables we can define in that universe. The distinction is relevant because we will have to use different distribution functions for the different populations. The number of electronic devices might be modeled with a Poisson distribution, but not with a normal distribution, because it is discrete and has a mean that is close to zero.
...
- The assignment of probabilities to events and random variables is strictly dependent on the details of the random experiment. We deal exclusively with random experiments where all unitary outcomes have equal probability, and where no events are rejected due to not being "representative."
```

The phrase "representative sample" is found frequently in books and articles, but in the vast majority of cases the term is used incorrectly. Statistics deals with random samples, not with "representative" samples. Excluding some very specific methods of sampling, there is no such thing as a "representative" sample. Most importantly, the validity of a sample should be based on the method used to obtain the sample and the methods and models used to make estimations and inferences based on the sample. In general, the validity of a sample should not be based on the characteristics of the sample assessed after the sample is obtained. Beware of "representative" samples. All the equations we will use in this book apply to random samples, not representative ones.
...
Incidentally, and to remark the importance of formulating questions carefully, the sampling mentioned by [@Pietra1926] was not suitable to estimate average family size by simply taking the sample average for the individuals selected by simple random sampling. However, it was correct to take the simple average to estimate the average size of family *experienced by individuals*. To further understand this issue, think about the average class size in UCD vs. the average class attended by students. How would you go about calculating each one if you had the complete database of courses and enrollments?
...
- Samples have to be obtained using a random process in which all units of the population have a strictly positive and known probability of being selected, even if the probabilities differ among units.
```
...
For the sample shown in Figure \@ref(fig:InferenceExample) the estimated variance of sample averages is obtained by first estimating the variance of the original population and then dividing by the sample size.:
...
A good candidate distribution would be the Poisson because the number of failures is a discrete number that can only be 0 or take positive values. The assumption of constant probability and independence of events meets the characteristics of the Poisson distribution.
...
3. An AI (artificial intelligence) sprayer detects individual weed plants with a probability of 0.90. If 3 or more plants are detected, it sprays an area equal to the size of the hoop and moves to the next area. What proportion of the area with 3 weed plants/hoop will be sprayed? Assume that detection of any individual plant is independent from detection of other plants.
