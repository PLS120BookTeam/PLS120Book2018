...
Main point: testing for significant differences is in a way a fool's errand, because unless we are actually sampling exactly the same populations, it is almost certain that the means will be different. When different sets of experimental units are treated differently, *something* is bound to change and most likely the means will be different. If means are different, we will **always** find that they are "significantly" different if we get large enough samples. This means that for the most part, the simple rejection of a null hyothesis of equality is not very informative.

```{bock, type = 'think'}

What!!?? If it is not very informative, why do we test whether means are equal??!! Because of historic reasons mostly, and a little due to misuse and misunderstanding of statistics by pracitioners. The good news is that the same methods are useful to test **better** hypothesis.

```

Statistical significance is not the same as biological or practical significance. This is clearly illustrated by an example. Suppose that you are interested in getting a phone with a long battery life. Model A costs $50 more and has a battery life that is statistically significantly longer than model B. The difference in battery life is 37.5 seconds.

So how do we make useful statistical statements and tests of hypotheses? Now, that is a great question! 

***** Add idea of least practically significant difference and testing null where the point is to determine if means differ at least by a certain amount, instead if they are different at all. COmplete with numerical example examined both ways: are measn different and are means practically different?*****
