...
1. Define experimental unit and determine the number of experimental units in an experiment.
...
```{r anova.intuition1, echo=FALSE, warning = FALSE, out.width='60%', fig.cap="Does it look like all groups come from the same parent population?"}
...
```{r anova.intuition2, echo=FALSE, warning = FALSE, out.width='60%', fig.cap="Does it look like all groups come from the same parent population?"}
...
&\bar{Y}{..} \quad \text{is the overall average calculated as} \quad \frac{1}{k}\sum_{i = 1}^{k} \bar{Y}_{i.}\ \\[15pt]
&\hat{\tau}_i \quad \text{is the estimated treatment effect for treatment i} \quad  \bar{Y}_{i.} - \bar{Y}_{..} \\[15pt]
&\hat{\epsilon}_{ij} \quad \text{is the estimated experimental error for treatment i and repetition j} \quad Y_{ij} - \bar{Y}_{i.} \\[15pt]
...
Based on the model above, each observation, $Y_{ij}$, is partitioned into three components:

- overall mean, $\hat{\mu}_{..}$, which is estimated with the average of treatment averages, $\bar{Y}_{..}$

- treatment effects, $\hat{\tau}_i$, which are estimated as the differences between treatment averages and overall average, $\bar{Y}_{i.} - \bar{Y}_{..}$

- error, $\hat{\epsilon}_{ij}$, which is estimated with the residual or the difference between individual observations and treatment averages, $Y_{ij} - \bar{Y}_{i.}$

<br>
\begin{align}
Y_{ij} &= \bar{Y}_{..} + \hat{\tau}_i + \hat{\epsilon}_{ij} \\[15pt]
Y_{ij}- \bar{Y}_{..} &= \hat{\tau}_i + \hat{\epsilon}_{ij} \\[15pt]
Y_{ij}- \bar{Y}_{..} &= (\bar{Y}_{i.} - \bar{Y}_{..})+ (Y_{ij} - \bar{Y}_{i.}) \\[15pt]
(\#eq:obsPartCRD)
\end{align}
<br>

The total deviation from observation to overall average is partitioned into the deviation from treatment average to overall average plus residual, where the residual is the difference between observation and treatment average. It can be shown that the sum of all total deviations squared equals the sum of squared estimated treatment effects plus the sum of squared residuals, thus the total sum of squares (TSS) equals the treatment sum of squares (SST) plus the residual sum of squares (SSE).
...
TSS &= SST + SSE\\[15pt]
\text{where} \\[15pt]
TSS &= Total Sum of Squares = \sum_{i=1}^k \sum_{j=1}^r (Y_{ij}- \bar{Y}_{..})^2 \\[15pt]
SST &= Treatment Sum of Squares = \sum_{i=1}^k \sum_{j=1}^r (\bar{Y}_{i.} - \bar{Y}_{..})^2 = \ r \ \sum_{i=1}^k (\bar{Y}_{i.} - \bar{Y}_{..})^2 \\[15pt]
SSE &= Residual Sum of Squares = \sum_{i=1}^k \sum_{j=1}^r (Y_{ij} - \bar{Y}_{i.})^2 \\[15pt]
(\#eq:ssPartCRD)
\end{align}
<br>
...
- If the calculated F is greater than critical F value, or if the probability of an F more extreme than calculated F is smaller than $\alpha$, we REJECT $H_0$.
...
#wgain (weight gain) is our response variable
#variety is our treatment
...
# Probability of F > Fcalc, when Fcalc = 1.8
pf(q = 1.8, df1 = 2, df2 = 9, lower.tail = FALSE)

```

This completes all the information in the ANOVA table. Now we proceed to make a confidence interval for an estimated treatment mean, for example for variety C. The statistics required to build the confidence interval are the same as before, specifying our treatment mean, $\bar{Y}_{i.}$, and calculating the standard error of the mean, $S_{\bar{Y}}$, and critical t-value, $t_{crit}$.

$${U \atop L} = \bar{Y_{i.}} \ \pm \ t_{crit} \cdot S_{\bar{Y}} $$
The standard error of the estimated mean is the square root of the MSE divided by the number of observations used to estimated the mean. In this case, all treatments have the same sample size or number of replications, and thus, all treatment standard errors are the same. However, in other experiments, sample sizes can differ among treatments and the standard error will be larger for the treatments with fewer observations.

$$S_\bar{Y} = \sqrt{\frac{MSE}{r}} = \sqrt{\frac{0.55556}{4}} = 0.372678$$
...
#the treatment average is corroborated when we used the `LSD.test` function.

```

Using the calculated values for the treatment average of variety C, the t critical value and the standard error of the means we can define the limits of our 95% confidence interval for variety C.
...
We cannot reject the null hypothesis that the means for varieties A and C are equal because of three equivalent facts (in addition to the nonsignifcant ANOVA test):
...
|--------------:|:--------------:|:--------------:|:--------------:|:---------:|:---------:|
|    A(28,28)   |      1.44      |      1.13      |      1.93      |    4.50   |    1.50   |
|    B(56,56)   |      1.34      |      1.11      |      1.87      |    4.32   |    1.44   |
|   C(112,112)  |      1.30      |      1.03      |      1.88      |    4.21   |    1.403  |
|   D(28,112)   |      1.33      |      1.00      |      1.87      |    4.20   |    1.4    |
...
This exercise has four parts. First, we read in and explore the data with some descriptive statistics. We check if we need to use a transformation to normalize the distribution of residuals. Second, each observation is partitioned into the components indicated by the ANOVA model and then the corresponding sums of squares (SS) and degreees of freedom (df) are computed. The ANOVA table and tests are done with the resulting values of SS and dfe. Third, we repeat the analysis using pre-existing R functions to do the ANOVA tables and tests directly. Finally, we calculate confidence intervals for treatment means an back-transform them to be able to interpret results in the original units of milk protein.
