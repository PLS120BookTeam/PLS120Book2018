...
Keep in mind that not being able to reject the null hypothesis is not the same as accepting it. The reason for this is that *it is very easy not to reject a null hypothesis*: all you have to do is not try hard enough, for example by obtaining a very samll sample. If not being able to reject the null meant that you accept it, then you would be able to accept any null hypothesis, which obviosuly is not very useful. It would simply lead you to be wrong most of the time.
...
Even though treatments were not significanlty different, we can calculate confidence intervals for the means and we can estimate the minimum difference between means that could be detected with the present variance and sample size. In all cases, the best estimate of the variance of the error we have is the MSE with dfe degrees of freedom.
...
Table \@ref(tab:aaAnovaTbl) shows that all estimated treatment means have the same SE and df of the error, which leads to identical confidence interval widths. The t-value for the confidence intervals, known as "critical" t or colloquialy "t from table" (as opposed to calculated t) is $t_{\ 0.975, \ 9} =$ `r qt(0.975,9)`
...
The last part of the ouput, a table of averages followed with letters is a standard way to present the results. Means that are followed by at least one letter in common are not significnatly different from each other. In this case there is only one letter following all the means because there are no significant differences.
...
1. get the standard error of estimated means and differences between estiamted means to test pairs of means or make confidence intervals.
...
We cannot reject the null hypothesis that themeans for varieties A and C are equal because of three equivalent facts (in addition to the nonsignifcant ANOVA test):
