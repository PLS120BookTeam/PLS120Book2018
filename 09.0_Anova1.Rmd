...
In the previous chapter, significant differences between two treatments were identified with t-tests.  However, there may be experiments when researchers want to test more than two treatments at the same time.  For example, a researcher may want to test the effects of 4 different fertilizer levels to compare yield, or a researcher may want to test the effects of 4 different diets on milk quality production in cows.  In these experiments, we can use the analysis of variance to compare the different sources of variation (treatments) to determine if any are significant.  

Analysis of Variance allows researchers to calculate important components of their experiment
1. partition the total variation (total sums of squares) into different sources of variation (treatments, residual/experimental error)
2. estimate the amount of variation per experimental unit from the different sources of variation
3. compare the different sources of variation using an F-test to determine if variances are equal

##Assumptions of ANOVA

Prior to running an ANOVA, there are assumptions that must all be met
1. the data collected must be normally distributed 
2. each experimental unit must be independent from any other experimental unit
3. the variances of the different treatments are homogeneous
4. the treatment and residual effects are additive
...
$Y_{ij} = \mu_{..} + \tau_i + \epsilon_{ij}$       where

$\mu{..}$ is the overall mean
$\tau_i$ is the treatment effect of treatment $i$ 
$\epsilon_{ij}$ is the experimental/residual error for treatment $i$ and repetition $j$
...
Among        $k-1$            $SST$                   $MST$             $\frac{MST}{MSE}$
Treatments

Within     $k(r-1)$           $SSE$                   $MSE$
Treatments
------------------------------------------------------------------------------------------
...
**Note: because our two treatments do not contain the same number of samples, 'NA' values are included to balance out the dataset in R.**
...
sample.avg <- sapply(fescue.var, mean)

#Sample Variances for Each Variety
sample.var <- sapply(fescue.var, var)
