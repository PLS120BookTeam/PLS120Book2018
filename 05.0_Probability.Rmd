...
Determine the probability mass function of a given random experiment.
List multiple causes that determine the outcome of a random experiment.
Classify a list of quantities as values of random variables or parameters.
Write down Bayes rule and define each of its components.
Apply Bayes rule to minimize error rate in classification of a plant as annual or perennial.

Concept of probability.
Concept of randomnesss.
Constant, Random variable and Random number. PSAES ch. 6

See probability_cheatsheet.pdf
...
3. Define "independence" in the context of probability events.  
4. Write out a sample space, given a set of conditions.  
5. Give an example of a random variable that can be modelled using a discrete distribution, and one using a continuous distribution.  
6. Emilio: would we have already covered central limit theorem, when you talked about the sum of uniform distributions (i think chp 2?), and if not, should we introduce it in this chapter, as the basis of p value and CI statistics, since we are contrasting frequentist with Bayesian stats?
...
See IntroductoryStatistics with R.

The following text was in sweave.
Needs to be translated into markdown.
