...
x <- 1:10
Y <- x +rnorm(10)
plot(x, Y,  pch = 20, xaxt = "n", yaxt = "n", xlab = "", ylab = "", xlim = c(0,10), ylim = c(0,10), bty = "l")
text(-1,5, "Y" , cex = 1.75)
...
As mentioned above, we use ordinary least squares to estimate the regression coefficients. This method minimizes the sum of the squared residuals. In other words, it finds the $\beta_0$ and $\beta_1$ that minimizes the squared distances of the observed values of Y from those predicted by the linear regression model, $hat{Y}$. With that in mind, we can use the following equations to estimate the regression coefficients, with r replicates: 

$$\beta_1 = \frac{\sum X_iY_i - r\overline{XY}}{\sum X_i^2 - r\overline{X}}^2$$

$$\beta_0 = \bar{Y} - \beta_1\bar{X}$$
##Analysis of variance of regression

In Chapter 11, we introduced you to a method called the analysis of variance.  To review, the general idea of analysis of variance is that we can partition the total variance of a Y dependent variable in order to test if there is a significant effect of an X independent variable on Y. In simple linear regression, we partition the total sum of squares of Y into the sum of squares that can be explained by the regression line and the sum of squares that cannot not explained by the regression line. In Figure \@ref(fig:anovaslr), we recycle Figure \@ref(fig:slr) to show you each component of variation in Y. The total variation in Y has to do with the deviation of each $Y_i$ from $\overline{Y}$. The regression line explains some of that variation in the deviations of each $\hat{Y}_i$ from $\overline{Y}$. The remaining, residual variation is in the deviations of each observation$Y_i$ from$\hat{Y}_i$.

```{r anovaslr, echo = F, fig.cap = ""}
########make a figure and label it slr#####
set.seed(400)
x <- 1:10
Y <- x +rnorm(10)
...
text(1.25, 3.75, expression(Y[i] - bar(Y)))
text(-1,5, "Y" , cex = 1.75)
text(5, -1.5,"X", cex = 1.75 )
lines(0:10, 0:10)
arrows(x0 = 6, y0 = 6, y1 = Y[6], code = 0,lty = 3, lwd = 1.15)
text(7, 5.5, labels = expression(hat(Y) - Y == epsilon[i]),cex = 1.5)
